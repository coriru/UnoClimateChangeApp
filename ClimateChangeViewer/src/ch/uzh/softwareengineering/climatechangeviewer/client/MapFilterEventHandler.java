...
public class MapFilterEventHandler extends Composite implements KeyDownHandler, MouseOverHandler, MouseOutHandler,
	...
	private FilterTooltip period1QueryTooltip = new FilterTooltip("Choose the starting year "
			+ "(min. " + QueryServiceImpl.OLDEST_YEAR_IN_DATAFILE + ") of the earlier period (range = 10 years) "
			+ "that shall be compared.");
	private FilterTooltip period2QueryTooltip = new FilterTooltip("Choose the starting year "
			+ "(max. " + (QueryServiceImpl.LATEST_YEAR_IN_DATAFILE - COMPARISON_PERIOD_LENGTH + 1) + ") "
			+ "of the later period (range = 10 years) that shall be compared.");
	private FilterTooltip uncertaintyQueryTooltip = new FilterTooltip("If entered, the data shown is restricted to "
			+ "the maximum average uncertainty of your choice.");
	...
	public boolean onSlide(SliderEvent event) {
		period1QueryValueLabel.setText(event.getValues()[0] + " - " + (event.getValues()[0]
				+ COMPARISON_PERIOD_LENGTH - 1));
		period2QueryValueLabel.setText(event.getValues()[1] + " - " + (event.getValues()[1]
				+ COMPARISON_PERIOD_LENGTH - 1));
		return true;
	}
